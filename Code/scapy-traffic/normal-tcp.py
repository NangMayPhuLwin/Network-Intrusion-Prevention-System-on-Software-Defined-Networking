# run on host's xterm terminal "python normal_tcp.py -s start_ip -e end_ip"
"""normal  traffic to the  subnet  with  the rate of   10packets   per seconds.
...
def gendest(start, end):
    # this function randomly generates IP addresses of the hosts based on entered start and end values
    first = 10
    second = 0
    third = 0
...
def main():
    ...
    for i in xrange(125):
        # form the packet
        payload = "send 8 normal TCP packets per one destination"
        packets = Ether()/IP(dst=gendest(start, end), src=sourceIPgen())/TCP(dport=80, sport=2)/payload
        print(repr(packets))
